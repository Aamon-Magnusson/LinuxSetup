...
function c
	cd (fdfind --type d | fzf --height=50 --border=rounded --prompt=">" --header="Select a directory to cd to" --header-first --reverse )
end

function ch
	cd ~/ && cd (fdfind --type d | fzf --height=50 --border=rounded --prompt=">" --header="Select a directory to cd to" --header-first --reverse )
end

function cc
	cd ~/.config && cd (fdfind --type d | fzf --height=50 --border=rounded --prompt=">" --header="Select a directory to cd to" --header-first --reverse )
end

function cpr # realized that cp is a command
	cd ~/Programming && cd (fdfind --type d | fzf --height=50 --border=rounded --prompt=">" --header="Select a directory to cd to" --header-first --reverse )
end

function cu
	cd ~/Drives && cd (fdfind --type d | fzf --height=50 --border=rounded --prompt=">" --header="Select a directory to cd to" --header-first --reverse )
end
...
abbr cdf "cd ~/Programming/DebianSetup"
abbr cdu "cd ~/Drives"
abbr cdc "cd ~/.config"
